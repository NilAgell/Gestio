...
if uploaded_file is not None:
    ...
    if 'superficieRealSol' in data.columns:
        df['Superficie Real Sól']= data['superficieRealSol']
    else: 
        df['Superficie Real Sól']= None
    ...
    if 'referenciaCadastral' in data.columns:
        df['Valor cadastral del sól'] = data['Total valor cadastral']
    else: 
        df['Valor cadastral del sól']= None

    if 'X' in data.columns:
        df['Coordenades X (Sistema de referència ETRS89)']= data['X']
    else: 
        df['Coordenades X (Sistema de referència ETRS89)'] =None
            
    if 'Y' in data.columns:
        df['Coordenades Y (Sistema de referència ETRS89)']= data['Y']
    else: 
        df['Coordenades Y (Sistema de referència ETRS89)'] =None
    ...
    for i in range(len(df)):
        if df['Comercialització'][i] == True:
            df.loc[i,'Comercialització'] = 'Si'
        else:
            df.loc[i,'Comercialització'] = 'No'
            
    for i in range(len(df)):
        if df['Dret de propietat'][i] == True:
    ...
    st.write("Identificació: Número de 7 xifres que permet identificar cada sól. \n ")
    st.write("Municipi: Nom del municipi. \n ")
    st.write("Codi municipi: Número de 5 xifres que permet identificar el municipi. \n ")
    st.write("Codi municipi: Número de 5 xifres que permet identificar el municipi. \n ")
    st.write("Codi municipi: Número de 5 xifres que permet identificar el municipi. \n ")
